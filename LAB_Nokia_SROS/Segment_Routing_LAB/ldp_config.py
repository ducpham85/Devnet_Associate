    ...
    for i in list_interface:
        ...
        for j in list_interface[i]:
            if j=='system':
                ...
                list_test.append(f'/configure router ldp interface-parameters interface {j}')
                list_test.append('/configure router ospf loopfree-alternates remote-lfa node-protect')
                list_test.append('/configure router ldp fast-reroute')
